...
Приложение разбито на три независимых компонента за единой точкой входа. Фронтенд — это чисто статика, он никогда не обращается к базе напрямую; все данные идут через бэкенд-API.
...
```
                  ...
                  │  (nginx / k8s ingress)│   маршрутизирует /web и /api
                  └─────┬───────────┬─────┘
                  /web/ │           │ /api/
                        ▼           ▼
              ┌─────────────────┐  ┌─────────────────┐
              │    Фронтенд     │  │     Бэкенд      │
              │   (todo-web)    │  │   (todo-api)    │
              │ статический HTML│  │  Go + Gin REST  │
              │ + Alpine.js + Tw│  │      API        │
              └─────────────────┘  └────────┬────────┘
                                            │ SQL (единственный путь к БД)
                                            ▼
                                   ┌─────────────────┐
                                   │   PostgreSQL    │
                                   └─────────────────┘
...
- **API Gateway** — единственный компонент, доступный снаружи. В Compose это nginx внутри контейнера `todo-web`; в Kubernetes — NGINX Ingress; на VPS — nginx на хосте ([ansible/roles/nginx/templates/todo.conf.j2](ansible/roles/nginx/templates/todo.conf.j2)).
- **Фронтенд (`todo-web`)** — статический HTML + Alpine.js + Tailwind, раздаётся через nginx. Не имеет ни кредов БД, ни DB-драйвера. Общается с бэкендом по HTTP через gateway.
- **Бэкенд (`todo-api`)** — REST-сервис на Go/Gin. Единственный компонент, который хранит креды БД и открывает соединения к PostgreSQL.
- **PostgreSQL** — не доступен через gateway. Достижим только из пода/контейнера бэкенда по внутренней сети.

Такое трёхуровневое разделение сохраняется во всех способах деплоя (Compose, Kind, GKE, VPS); меняется только реализация gateway и хостинг БД.
